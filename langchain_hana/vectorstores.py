...
class HanaDB(VectorStore):
    ...
    def _similarity_search_with_score_and_vector(
        ...
        additional_metadata_cols = projected_metadata_columns or self.specific_metadata_columns
        additional_metadata_sql = ", ".join(f'"{col}"' for col in additional_metadata_cols)
        ...
        # Append additional metadata columns if any
        if additional_metadata_sql:
            sql_str += ", " + additional_metadata_sql
            
        sql_str += f" FROM {from_clause}"   
        ...
        try:
            ...
            if cur.has_result_set():
                rows = cur.fetchall()
                for row in rows:
                    js = json.loads(row[1]) if row[1] else {}
                    # Extract additional metadata columns (after row[3])
                    additional_values = row[4:]
                    for idx, col in enumerate(additional_metadata_cols):
                        js[col] = additional_values[idx]
